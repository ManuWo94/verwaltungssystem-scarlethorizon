...
// Lade Rollen für die Rollenzuweisung
$rolesFile = '../data/roles.json';
$allRoles = getJsonData($rolesFile);
if ($allRoles === false) {
    $allRoles = [];
}
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {
    ...
    if ($action === 'claim_task') {
        ...
        foreach ($tasks as $key => $task) {
            if ($task['id'] === $taskId) {
                ...
                // Prüfe, ob die Aufgabe einer Rolle zugewiesen ist
                if (empty($task['assignment_type']) || $task['assignment_type'] !== 'role') {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Diese Aufgabe kann nicht beansprucht werden (keine Rollenzuweisung).'
                    ]);
                    exit;
                }
                ...
                $tasks[$key]['claimed_by'] = $user_id;
                $tasks[$key]['claimed_at'] = date('Y-m-d H:i:s');
                ...
                if (saveJsonData($tasksFile, $tasks)) {
                    ...
                    foreach ($users as $u) {
                        if ($u['id'] === $user_id) {
                            $claimedByName = isset($u['first_name']) && isset($u['last_name']) ? 
                    ...
                    echo json_encode([
                        'success' => true,
                        'message' => 'Aufgabe wurde erfolgreich beansprucht.',
                        'task' => $tasks[$key],
    ...
    // Aufgabe freigeben (Unclaim)
    elseif ($action === 'unclaim_task') {
        $taskId = isset($_POST['task_id']) ? sanitize($_POST['task_id']) : '';
        $taskFound = false;
        ...
        foreach ($tasks as $key => $task) {
            if ($task['id'] === $taskId) {
                ...
                // Prüfe, ob der Benutzer die Aufgabe beansprucht hat
                if (empty($task['claimed_by']) || $task['claimed_by'] !== $user_id) {
                    // Admins können Aufgaben freigeben
                    if (!$canAssignTasks) {
                        echo json_encode([
                            'success' => false,
                            'message' => 'Sie haben diese Aufgabe nicht beansprucht.'
                        ]);
                        exit;
                    }
                }
                
                // Gebe die Aufgabe frei
                $tasks[$key]['claimed_by'] = null;
                $tasks[$key]['claimed_at'] = null;
                $tasks[$key]['updated_at'] = date('Y-m-d H:i:s');
                $tasks[$key]['updated_by'] = $user_id;
                ...
                if (saveJsonData($tasksFile, $tasks)) {
                    echo json_encode([
                        'success' => true,
                        'message' => 'Aufgabe wurde erfolgreich freigegeben.',
                        'task' => $tasks[$key]
                    ]);
                } else {
                    echo json_encode([
    ...
    elseif ($action === 'toggle_task_status') {
        ...
        foreach ($tasks as $key => $task) {
            if ($task['id'] === $taskId) {
                ...
                // Bei Rollenaufgaben: Nur wer die Aufgabe beansprucht hat, kann sie abhaken
                if (!empty($task['assignment_type']) && $task['assignment_type'] === 'role') {
                    if (!empty($task['claimed_by']) && $task['claimed_by'] === $user_id) {
                        $canEdit = true;
                    } elseif ($canAssignTasks || $task['created_by'] === $user_id) {
                        // Admins und Ersteller können auch abhaken
                        $canEdit = true;
                    }
                } else {
                    // Bei Benutzerzuweisung: Zugewiesener, Ersteller oder Berechtigter
                ...
                if (!$canEdit) {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Sie haben keine Berechtigung, diese Aufgabe abzuhaken. Bei Rollenaufgaben müssen Sie die Aufgabe zuerst beanspruchen.'
                    ]);
                    exit;
                }
    ...
    elseif ($action === 'get_task') {
        ...
        foreach ($tasks as $task) {
            if ($task['id'] === $taskId) {
                ...
                // GEÄNDERT: Alle Benutzer können Aufgaben sehen (für Kommentare)
                // Prüfe, ob der Benutzer die Aufgabe sehen darf
                $canView = true; // Jeder kann alle Aufgaben sehen
                ...
                // Füge Namen des Zugewiesenen hinzu (Benutzer oder Rolle)
                $assignedToName = "Unbekannt";
                if (!empty($task['assignment_type']) && $task['assignment_type'] === 'role') {
                    // Finde Rollennamen
                    foreach ($allRoles as $roleData) {
                        if ($roleData['id'] === $task['assigned_to'] || $roleData['name'] === $task['assigned_to']) {
                ...
                // Füge Namen des Beanspruchenden hinzu (falls Rollenzuweisung)
                if (!empty($task['claimed_by'])) {
                    $claimedByName = "Unbekannt";
                    foreach ($users as $u) {
                        if ($u['id'] === $task['claimed_by']) {
                            $claimedByName = isset($u['first_name']) && isset($u['last_name']) ? 
                                          $u['first_name'] . ' ' . $u['last_name'] : $u['username'];
                            break;
                        }
                    }
                    $task['claimed_by_name'] = $claimedByName;
                }
    ...
    elseif ($action === 'add_comment') {
        ...
        foreach ($tasks as $key => $task) {
            if ($task['id'] === $taskId) {
                ...
                // GEÄNDERT: Alle können kommentieren
                $canComment = true;
            ...
            if (empty($assignedTo)) {
                ...
                if ($assigneeExists) {
                    ...
                    $taskData = [
                        ...
                        'priority' => $priority,
                        'assignment_type' => $assignmentType, // 'user' oder 'role'
                        'assigned_to' => $assignedTo,
                        'assigned_to_name' => $assignedToName,
                        'claimed_by' => null, // Nur für Rollenaufgaben relevant
                        'claimed_at' => null,
                        'due_date' => $dueDate,
                        'completed' => false,
                        'completed_at' => null,
                    ...
                    if (saveJsonData($tasksFile, $tasks)) {
                        // Bei Rollenzuweisung: Benachrichtige alle Benutzer mit dieser Rolle
                        if ($assignmentType === 'role') {
                            try {
                                ...
                                foreach ($users as $u) {
                                    // Prüfe, ob der Benutzer die Rolle hat
                                    $userHasRole = false;
                                    if (isset($u['roles']) && is_array($u['roles'])) {
                                        foreach ($allRoles as $roleData) {
                                            if (($roleData['id'] === $assignedTo || $roleData['name'] === $assignedTo) &&
                                                (in_array($roleData['name'], $u['roles']) || 
                                                 (isset($u['role']) && $u['role'] === $roleData['name']))) {
                                                $userHasRole = true;
                                                break;
                                            }
                                        }
                                    } elseif (isset($u['role_id']) && $u['role_id'] === $assignedTo) {
                                        $userHasRole = true;
                                    }
                                    
                                    if ($userHasRole && $u['id'] !== $user_id) {
                                        @createNotification(
                                            $u['id'],
                                            'task',
                                            'Neue Rollenaufgabe verfügbar',
                                            $creatorName . ' hat eine neue Aufgabe "' . $taskData['title'] . '" für Ihre Rolle erstellt. Sie können diese Aufgabe beanspruchen.',
                            ...
                            $message = 'Aufgabe wurde erfolgreich erstellt und der Rolle ' . str_replace(' (Rolle)', '', $assignedToName) . ' zugewiesen.';
                        } else {
    ...
    elseif ($action === 'delete_task' && isset($_POST['task_id'])) {
        ...
        foreach ($tasks as $key => $task) {
            if ($task['id'] === $taskId) {
                ...
                // Nur Ersteller oder Admins können löschen
                $canDelete = ($task['created_by'] === $user_id || $canAssignTasks);
                
                if (!$canDelete) {
                    $error = 'Nur der Ersteller oder ein Administrator kann diese Aufgabe löschen.';
                    break;
                }
...
$visibleTasks = [];
foreach ($tasks as $task) {
    // GEÄNDERT: Alle Benutzer können alle Aufgaben sehen (für Kommentare und Transparenz)
    // Bei Rollenaufgaben: Zeige nur Aufgaben der eigenen Rollen oder wenn man Admin/Ersteller ist
    $canSee = false;
    
    // Admins und Ersteller sehen immer
    if ($canAssignTasks || $task['created_by'] === $user_id) {
        $canSee = true;
    }
    // Aufgaben, die direkt zugewiesen sind
    elseif (empty($task['assignment_type']) || $task['assignment_type'] === 'user') {
        if ($task['assigned_to'] === $user_id) {
            $canSee = true;
        }
    }
    // Rollenaufgaben: Zeige, wenn Benutzer die Rolle hat
    elseif ($task['assignment_type'] === 'role') {
        $assignedRoleId = $task['assigned_to'];
        foreach ($allRoles as $roleData) {
    ...
    // Auch Aufgaben sichtbar machen, die man beansprucht hat
    if (!empty($task['claimed_by']) && $task['claimed_by'] === $user_id) {
        $canSee = true;
    }
                                ...
                                <tbody>
                                    <?php foreach ($visibleTasks as $task): ?>
                                        ...
                                        <tr class="task-item <?php echo $task['completed'] ? 'task-completed text-muted' : ''; ?>" 
                                            ...
                                            <td>
                                                <?php 
                                                // Zeige Rollenzuweisung oder Benutzerzuweisung
                                                echo htmlspecialchars($task['assigned_to_name']); 
                                                
                                                // Zeige Beanspruchung bei Rollenaufgaben
                                                if (!empty($task['assignment_type']) && $task['assignment_type'] === 'role'): 
                                                    if (!empty($task['claimed_by'])): 
                                                        // Finde Namen des Beanspruchenden
                                                        $claimedByName = 'Unbekannt';
                                                        foreach ($users as $u) {
                                                            if ($u['id'] === $task['claimed_by']) {
                                                                $claimedByName = isset($u['first_name']) && isset($u['last_name']) ? 
                                                ...
                                                ?>
                                                        <br><small class="text-success">
                                                            <i class="fas fa-user-check"></i> Beansprucht von: <?php echo htmlspecialchars($claimedByName); ?>
                                                            <?php if ($task['claimed_by'] === $user_id): ?>
                                                                (Sie)
                                                            <?php endif; ?>
                                                        </small>
                                                    <?php else: ?>
                                                        <br><small class="text-warning">
                                                            <i class="fas fa-clock"></i> Noch nicht beansprucht
                                                        </small>
                                                    <?php endif; 
                                                endif;
                                                
                                                // Zeige "Sie" bei direkter Zuweisung
                                                if ((empty($task['assignment_type']) || $task['assignment_type'] === 'user') && $task['assigned_to'] === $user_id): ?>
                                            ...
                                            <td>
                                                <?php 
                                                    ...
                                                    $userCanDeleteTask = $canAssignTasks || $task['created_by'] === $user_id;
                                                    
                                                    // Prüfe Claim-Berechtigung für Rollenaufgaben
                                                    $canClaim = false;
                                                    $canUnclaim = false;
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <?php if ($canClaim): ?>
                                                        <button class="btn btn-sm btn-success claim-task-btn" 
                                                                data-task-id="<?php echo htmlspecialchars($task['id']); ?>" 
                                                                title="Aufgabe beanspruchen">
                                                            <i class="fas fa-hand-paper"></i>
                                                        </button>
                                                    <?php endif; ?>
                                                    
                                                    <?php if ($canUnclaim): ?>
                                                        <button class="btn btn-sm btn-warning unclaim-task-btn" 
                                                                data-task-id="<?php echo htmlspecialchars($task['id']); ?>" 
                                                                title="Beanspruchung aufheben">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="assignment_type" id="assignment_type_user" value="user" checked>
                            <label class="form-check-label" for="assignment_type_user">
                                Benutzer zuweisen
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="assignment_type" id="assignment_type_role" value="role">
                            <label class="form-check-label" for="assignment_type_role">
                                Rolle zuweisen (alle Benutzer mit dieser Rolle können die Aufgabe beanspruchen)
                            </label>
                        </div>
                    </div>
                    
                    <div class="form-group" id="user_assignment_group">
                        <label for="assigned_to">Benutzer auswählen *</label>
                    ...
                    <div class="form-group" id="role_assignment_group" style="display: none;">
                        <label for="assigned_to_role">Rolle auswählen *</label>
                        <select class="form-control" id="assigned_to_role" name="assigned_to_role">
                            <option value="">-- Bitte auswählen --</option>
                            <?php foreach ($allRoles as $roleData): ?>
                                <option value="<?php echo htmlspecialchars($roleData['id']); ?>">
                                    <?php echo htmlspecialchars($roleData['name']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <small class="form-text text-muted">
                            Alle Benutzer mit dieser Rolle sehen die Aufgabe und können sie beanspruchen.
                        </small>
                    </div>
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Zugewiesen an
                    let assigneeHtml = '<i class="fas fa-user mr-1"></i> ' + task.assigned_to_name;
                    
                    // Zeige Beanspruchungsinformationen bei Rollenaufgaben
                    if (task.assignment_type === 'role') {
                        if (task.claimed_by) {
                            assigneeHtml += '<br><small class="text-success"><i class="fas fa-user-check"></i> Beansprucht von: ' + 
                                          (task.claimed_by_name || 'Unbekannt') + '</small>';
                        } else {
                            assigneeHtml += '<br><small class="text-warning"><i class="fas fa-clock"></i> Noch nicht beansprucht</small>';
                        }
                    }
                    
                    $('#taskAssignee').html(assigneeHtml);
    ...
    // Aufgabe beanspruchen (Claim)
    $(document).on('click', '.claim-task-btn', function(e) {
        e.preventDefault();
        const taskId = $(this).data('task-id');
        const btn = $(this);
        
        if (!confirm('Möchten Sie diese Aufgabe beanspruchen?')) {
            return;
        }
        ...
        $.ajax({
            ...
            error: function() {
                showAlert('danger', 'Ein Fehler ist aufgetreten. Bitte versuchen Sie es später erneut.');
                btn.prop('disabled', false);
            }
        });
    });
    ...
    // Aufgabe freigeben (Unclaim)
    $(document).on('click', '.unclaim-task-btn', function(e) {
        e.preventDefault();
        const taskId = $(this).data('task-id');
        const btn = $(this);
        
        if (!confirm('Möchten Sie die Beanspruchung dieser Aufgabe aufheben?')) {
            return;
        }
    ...
    // Radio-Button Wechsel für Zuweisungstyp
    $('input[name="assignment_type"]').on('change', function() {
        const assignmentType = $(this).val();
        
        if (assignmentType === 'user') {
            $('#user_assignment_group').show();
            $('#role_assignment_group').hide();
            $('#assigned_to').prop('required', true);
            $('#assigned_to_role').prop('required', false);
        } else if (assignmentType === 'role') {
            $('#user_assignment_group').hide();
            $('#role_assignment_group').show();
            $('#assigned_to').prop('required', false);
            $('#assigned_to_role').prop('required', true);
        }
    });
    
    // Beim Absenden des Formulars den richtigen Wert setzen
    $('#createTaskModal form').on('submit', function(e) {
        const assignmentType = $('input[name="assignment_type"]:checked').val();
        
        if (assignmentType === 'role') {
            // Kopiere den Wert von assigned_to_role nach assigned_to
            const roleId = $('#assigned_to_role').val();
            $('#assigned_to').val(roleId);
        }
    });
