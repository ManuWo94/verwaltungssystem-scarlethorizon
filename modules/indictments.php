...
// Debug-Log nach der Kategorisierung
error_log("Pending indictments count: " . count($pendingIndictments));
error_log("Accepted indictments count: " . count($acceptedIndictments));
error_log("Scheduled indictments count: " . count($scheduledIndictments));
error_log("Rejected indictments count: " . count($rejectedIndictments));
error_log("Completed indictments count: " . count($completedIndictments));
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <div>
                                                    <?php if ($selectedIndictment['status'] === 'accepted'): ?>
                                                        ...
                                                        <button type="button" class="btn btn-primary mb-3" id="scheduleCourtBtn" 
                                                                onclick="toggleScheduleForm()">
                                                            <i class="fas fa-calendar-alt"></i> Gerichtsverhandlung terminieren
                                                        </button>
                                                        
                                                        <!-- Eingebettetes Formular statt Modal-Dialog -->
                                                        <div id="scheduleCourtForm" style="display: none;" class="border rounded p-3 mb-3 bg-light">
                                                            <h5 class="mb-3">Verhandlungstermin festlegen</h5>
                                                            <form method="post" action="indictments.php">
                                                                <input type="hidden" name="action" value="schedule_court_date">
                                                                <input type="hidden" name="indictment_id" value="<?php echo $selectedIndictment['id']; ?>">
                                                                
                                                                <div class="form-group row">
                                                                    <label for="trial_date" class="col-sm-3 col-form-label"><strong>Datum:</strong></label>
                                                                    <div class="col-sm-9">
                                                                ...
                                                                <div class="form-group row">
                                                                    <label for="trial_time" class="col-sm-3 col-form-label"><strong>Uhrzeit:</strong></label>
                                                                    <div class="col-sm-9">
                                                                ...
                                                                <div class="form-group row">
                                                                    <label for="trial_notes" class="col-sm-3 col-form-label"><strong>Anmerkungen:</strong></label>
                                                                    <div class="col-sm-9">
                                                                ...
                                                                <div class="form-group row">
                                                                    <div class="col-sm-12 text-right">
                                                                        <button type="button" class="btn btn-secondary mr-2" onclick="toggleScheduleForm()">Abbrechen</button>
                            ...
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    ...
                                    <?php if (count($scheduledIndictments) == 0): ?>
                                        <p class="text-muted">Keine terminierten Verhandlungen gefunden.</p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal fade" id="enterVerdictModal" tabindex="-1" role="dialog" aria-labelledby="enterVerdictModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <form id="verdictForm" method="post" action="indictments.php">
                                    <div class="modal-body">
                                        <input type="hidden" name="action" value="enter_verdict">
                                        <input type="hidden" id="verdict_indictment_id" name="indictment_id" value="">
...
<script>
// Funktion zum Ein-/Ausblenden des Terminierungs-Formulars
function toggleScheduleForm() {
    var form = document.getElementById('scheduleCourtForm');
    if (form) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Wir nutzen jetzt den Bootstrap data-toggle Mechanismus für die Modals
    // Bei Öffnen des Modals übertragen wir die ID aus dem Button in das Form-Feld
    $('#scheduleCourtModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget); // Button, der das Modal ausgelöst hat
        var indictmentId = button.data('indictment-id'); // Hole die Indictment-ID aus dem data-Attribut
        console.log('Gerichtsverhandlung terminieren für ID:', indictmentId);
        
        // Finde das versteckte Input-Feld im Formular
        var modal = $(this);
        var idInput = modal.find('#schedule_indictment_id');
        if (idInput.length) {
            idInput.val(indictmentId);
            console.log('Indictment ID im Formular gesetzt auf:', idInput.val());
        } else {
            console.error('Konnte das Input-Feld mit ID schedule_indictment_id nicht finden');
        }
    });
    ...
    if (enterVerdictBtn && enterVerdictModal) {
        enterVerdictBtn.onclick = function(e) {
            e.preventDefault();
            const indictmentId = this.getAttribute('data-indictment-id');
            console.log('Urteil eintragen für ID:', indictmentId);
            
            // Finde das versteckte Input-Feld im Formular
            const idInput = document.getElementById('verdict_indictment_id');
            if (idInput) {
                idInput.value = indictmentId;
                console.log('Indictment ID im Urteilsformular gesetzt auf:', idInput.value);
            } else {
                console.error('Konnte das Input-Feld mit ID verdict_indictment_id nicht finden');
            }
            
            // Öffne das Modal mit jQuery
